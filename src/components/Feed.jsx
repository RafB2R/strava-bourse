...
function getActivityMeta(activity) {
  ...
  return map[activity.type] || { tag: "Activité", tagBg: "rgba(255,255,255,0.05)", tagColor: "rgba(255,255,255,0.4)", title: `${name} a eu une activité`, sub: "", stat: "" };
}
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1 }}>
            <textarea
              ...
              style={{ width: "100%", background: "none", border: "none", outline: "none", color: "#f0f0f0", fontFamily: "inherit", fontSize: 14, resize: "none", lineHeight: 1.5, minHeight: 60 }}
            />
            {postInput.trim() && (
              <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 8 }}>
                <button onClick={publishPost} disabled={posting} style={{ background: "#9FE1CB", border: "none", borderRadius: 999, padding: "6px 18px", fontSize: 13, fontWeight: 700, color: "#0F6E56", cursor: "pointer", fontFamily: "inherit" }}>
                  {posting ? "…" : "Publier"}
                </button>
              </div>
            )}
        ...
        return (
          <div key={activity.id} style={card}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: "#fff", cursor: "pointer" }} onClick={() => setSelectedUser(activity.user_id)}>
            ...
            {openComment[activity.id] && (
              <div style={{ marginTop: 12, borderTop: "0.5px solid rgba(255,255,255,0.06)", paddingTop: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <input value={commentInputs[activity.id] || ""} onChange={e => setCommentInputs(p => ({ ...p, [activity.id]: e.target.value }))} onKeyDown={e => e.key === "Enter" && addComment(activity.id)} placeholder="Commenter…" style={{ flex: 1, padding: "7px 10px", fontSize: 13, borderRadius: 8, border: "0.5px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.04)", color: "#f0f0f0", fontFamily: "inherit" }} />
