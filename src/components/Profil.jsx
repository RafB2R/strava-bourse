...
function StatsSection({ profile, session, friends, perf }) {
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.04)", border: "0.5px solid rgba(255,255,255,0.08)", borderRadius: 14, padding: "1.25rem", marginBottom: 12 }}>
      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", fontWeight: 500, marginBottom: 12, textTransform: "uppercase", letterSpacing: "0.05em" }}>Classement amis</div>
...
const btnGreen = { background: "none", border: "0.5px solid #9FE1CB", borderRadius: 8, padding: "5px 10px", fontSize: 12, color: "#9FE1CB", cursor: "pointer", fontFamily: "inherit" };
const btnRed = { background: "none", border: "0.5px solid #F08080", borderRadius: 8, padding: "5px 10px", fontSize: 12, color: "#F08080", cursor: "pointer", fontFamily: "inherit" };
...
export default function Profil({ profile: initialProfile, session }) {
    ...
    <div>
      <div style={card}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
          <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(159,225,203,0.12)", color: "#9FE1CB", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, flexShrink: 0 }}>{initials}</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 17, fontWeight: 700, color: "#fff" }}>{profile.full_name || "—"}</div>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>@{profile.username || "—"}{profile.city ? ` · ${profile.city}` : ""}</div>
            {profile.strategy && <span style={{ display: "inline-block", marginTop: 4, padding: "2px 8px", borderRadius: 999, fontSize: 11, fontWeight: 500, background: "rgba(159,225,203,0.1)", color: "#9FE1CB" }}>{profile.strategy}</span>}
          </div>
          <button style={btnSm} onClick={startEdit}>✏️ Éditer</button>
        </div>

        {profile.bio && <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", lineHeight: 1.6, marginBottom: 16, padding: "10px 12px", background: "rgba(255,255,255,0.03)", borderRadius: 8 }}>{profile.bio}</div>}

        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }}>
          {[
            ["Positions", stats.positions, "#9FE1CB"],
            ["Perf. totale", perf === null ? "—" : `${perf >= 0 ? "+" : ""}${perf.toFixed(2)}%`, perf === null ? "rgba(255,255,255,0.3)" : perf >= 0 ? "#9FE1CB" : "#F08080"],
            ...
            <div key={label} style={{ background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: 12 }}>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", marginBottom: 4 }}>{label}</div>
              <div style={{ fontSize: 20, fontWeight: 700, color }}>{val}</div>
            </div>
          ))}
        </div>
        {saved && <div style={{ marginTop: 12, padding: "8px 12px", background: "rgba(159,225,203,0.1)", borderRadius: 8, fontSize: 13, color: "#9FE1CB", textAlign: "center" }}>✅ Profil mis à jour !</div>}
      </div>

      {editing && (
        <div style={card}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "#fff", marginBottom: 16 }}>✏️ Modifier mon profil</div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
        {[["stats", "📊 Stats"], ["badges", `🏅 Badges (${unlockedCount})`], ["reseau", "👥 Réseau"]].map(([id, label]) => (
          <button key={id} onClick={() => setSection(id)} style={{ padding: "6px 14px", borderRadius: 999, fontSize: 12, border: `0.5px solid ${section === id ? "#9FE1CB" : "rgba(255,255,255,0.1)"}`, background: section === id ? "rgba(159,225,203,0.1)" : "none", color: section === id ? "#9FE1CB" : "rgba(255,255,255,0.4)", cursor: "pointer", fontFamily: "inherit" }}>
            {label}
          </button>
        ))}
      </div>
      ...
      {section === "stats" && (
        <StatsSection profile={profile} session={session} friends={friends} perf={perf} />
      )}
      ...
      {section === "reseau" && (
        <div>
          <div style={card}>
            ...
            {message && <div style={{ fontSize: 13, color: "#9FE1CB", marginBottom: 10 }}>{message}</div>}
            {searching && <div style={{ fontSize: 13, color: "rgba(255,255,255,0.3)" }}>Recherche…</div>}
            {searchResults.map(u => (
              <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderTop: "0.5px solid rgba(255,255,255,0.06)" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)" }}>@{u.username}</div>
                </div>
                {alreadyIds.includes(u.id) ? <span style={{ fontSize: 12, color: "rgba(255,255,255,0.25)" }}>Déjà ajouté</span> : <button style={btnGreen} onClick={() => sendRequest(u.id)}>+ Ajouter</button>}
              </div>
            ))}
            {search.length >= 2 && !searching && searchResults.length === 0 && <div style={{ fontSize: 13, color: "rgba(255,255,255,0.3)" }}>Aucun résultat</div>}
          </div>
